...
for value_name in AUTO_ACCEPT_SYSTEM AUTO_ACCEPT_PIP AUTO_ACCEPT_CONTAINER REQUIRE_PINNED_IMAGE KEEP_BUILD NO_PUBLISH IN_CONTAINER; do
  value="${!value_name:-0}"
...
build_local_target() {
  ...
  # compiled nexs_ledger launcher. CPython prepends the script's own
  # directory to sys.path[0], so a user who cd's into the extracted
  # directory and runs the system python3 (a different minor version,
  # e.g. Homebrew 3.13) would dlopen the bundle's CPython 3.12 native
  # extension modules against a mismatched interpreter and crash with
  # missing-symbol errors. test.py now refuses to run in that state
  # (see its top-of-file guard); this generated wrapper performs the
  # correct invocation for the user, so the released artifact is
  # self-serviceable without needing to read this script's source.
  #
  # The wrapper is deliberately POSIX /bin/sh only (no bash, no
  # GNU-isms): it must work on macOS, Linux glibc and Linux musl,
  # which are the only targets this function ever builds. It uses
  # `exec env PYTHONSAFEPATH=1` (documented, portable across POSIX
  # shells and CPython 3.11+) rather than `-P`, because -P is not
  # available on CPython 3.10 and older and we do not want to
  # silently fall back to an unsafe invocation on those interpreters.
  # It also honours $PYTHON if the user has exported one, and passes
  # any extra args through to test.py so `./run-test.sh --help` etc.
  # still work.
  # -------------------------------------------------------------------
  cat > "$package/run-test.sh" <<'NEXS_RUN_TEST_SH_EOF'
#!/bin/sh
# Auto-generated by nexs_build_tools/build_release.sh.
#
# Run the packaged test.py in safe-path mode against the packaged
# nexs_ledger. See the top of test.py for why this wrapper exists and
# why invoking test.py directly from this directory with an
# unsandboxed stock python3 is not safe.
set -eu
cd -- "$(dirname -- "$0")"
PY="${PYTHON:-python3}"
exec env PYTHONSAFEPATH=1 "$PY" ./test.py --binary ./nexs_ledger --source ./main.py "$@"
NEXS_RUN_TEST_SH_EOF
  chmod 0755 "$package/run-test.sh"
  # -------------------------------------------------------------------
...
publish_release() {
  ...
  [[ ! -e "$RELEASE_DIR/RELEASE_CATALOGUE-$VERSION.json" && ! -e "$RELEASE_DIR/BINARY_HASH_CATALOGUE-$VERSION.json" && ! -e "$RELEASE_DIR/SHA256SUMS-$VERSION" ]] || die "Release version already exists. Refusing overwrite."
